...
// therefore, it will compile longer, but can be slightly faster
  ...
  inline static void apply(T*__restrict x) {
    ...
    if ((NUM_BITS & 1) == 1) {
      ...
      for (k=0; k<SUB_N; k+=4) {
      	...
      	SemiRecursiveShuffle<T, SUB_NUM_BITS, RECURSIONS_REMAINING-1>::apply(x+(1<<SUB_NUM_BITS)+(k<<SUB_NUM_BITS));
      	SemiRecursiveShuffle<T, SUB_NUM_BITS, RECURSIONS_REMAINING-1>::apply(x+(2<<SUB_NUM_BITS)+(k<<SUB_NUM_BITS));
      	SemiRecursiveShuffle<T, SUB_NUM_BITS, RECURSIONS_REMAINING-1>::apply(x+(3<<SUB_NUM_BITS)+(k<<SUB_NUM_BITS));
      }
      ...
      // 	{
      // 	for (k=0; k<SUB_N/4; k++) {
      // 	  SemiRecursiveShuffle<T, SUB_NUM_BITS, RECURSIONS_REMAINING-1>::apply((x+((j*SUB_N/4)<<SUB_NUM_BITS))+(k<<SUB_NUM_BITS));
      // 	}
      // 	}
