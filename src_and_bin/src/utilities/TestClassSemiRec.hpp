...
template<template<typename , unsigned char, unsigned char> class Shuffler, typename T, unsigned char LOG_N, unsigned char RECURSIONS_REMAINING>
class TestRuntime{
public:
	static void run(int runs, float quant){
		...
		for (int i = 0; i < runs; ++i ){
			c.tick();
			Shuffler<T, LOG_N, RECURSIONS_REMAINING>::apply(testArray);
			times[i] = c.tock();
		};
